...
export async function POST(request: NextRequest) {
  try {
    ...
    // Construct the main QC path (where the file should be)
    ...
    const mainQcPath = useCustomStructure
      ? `QC/${fileName}`
      : `${sanitizedClientName}/QC/${fileName}`;
    const mainQcStorageUrl = `https://se.storage.bunnycdn.com/${finalStorageZone}/${mainQcPath}`;
    const mainQcPublicUrl = `${cdnUrl}/${mainQcPath}`;
    ...
    console.log("📥 Downloading backup file from:", backupStorageUrl);
    ...
    // Upload the backup file to main QC location (overwrites current file)
    // Add cache-control headers to prevent caching of the restored file
    console.log("📤 Uploading restored file to:", mainQcStorageUrl);
    console.log("📊 File size:", fileBuffer.byteLength, "bytes");
    const uploadResponse = await fetch(mainQcStorageUrl, {
      method: "PUT",
      headers: {
        AccessKey: customAccessKey,
    ...
    if (!uploadResponse.ok) {
      ...
      return NextResponse.json(
        {
          error: "Failed to copy file to main QC location",
    ...
    const verifyResponse = await fetch(mainQcStorageUrl, {
      method: "GET",
      headers: {
        AccessKey: customAccessKey,
    ...
    // Purge cache for the restored GLB file URL
    if (bunnyApiKey) {
      try {
        console.log("🗑️ Purging cache for restored GLB:", mainQcPublicUrl);
        const purgeResponse = await fetch(
          "https://api.bunny.net/purge?async=false",
          {
            ...
            body: JSON.stringify({ urls: [mainQcPublicUrl] }),
          }
        );

        if (purgeResponse.ok) {
          console.log("✅ Cache purged successfully for:", mainQcPublicUrl);
        } else {
    ...
    // Update the asset in the database with the restored GLB link
    const timestamp = Date.now();
    const { error: updateError } = await supabase
      .from("onboarding_assets")
      .update({
        glb_link: mainQcPublicUrl, // Store clean URL without cache-busting in DB
    ...
    return NextResponse.json({
      success: true,
      glbUrl: mainQcPublicUrl,
      restoreTimestamp: timestamp, // Include timestamp for cache-busting
      message: "File restored to main QC location",
    });
  } catch (error) {
    console.error("❌ Error restoring GLB file:", error);
    return NextResponse.json(
