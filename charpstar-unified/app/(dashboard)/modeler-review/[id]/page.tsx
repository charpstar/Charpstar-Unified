...
export default function ModelerReviewPage() {
  ...
  const [automatedQARunning, setAutomatedQARunning] = useState(false);
  const [qaStatus, setQaStatus] = useState<string>("");
  const [qaError, setQaError] = useState<string>("");
  const [qaJobId, setQaJobId] = useState<string>("");
  const [qaAnalysisJobId, setQaAnalysisJobId] = useState<string | null>(null);
  const qaPollTimerRef = useRef<NodeJS.Timeout | null>(null);
  ...
  const cleanupGlbHistory = async () => {
    ...
    try {
      ...
      if (currentHistory && currentHistory.length > 1) {
        ...
        const urlGroups = currentHistory.reduce((groups, item) => {
  ...
  const hiddenViewerRef = useRef<any>(null);

  const handleAutomatedQA = async () => {
    if (!asset?.glb_link) {
      toast.error("No GLB file available for QA");
      return;
    }

    setAutomatedQARunning(true);
    setQaError("");
    setQaStatus("Initializing automated QA...");

    try {
      // Create QA job
      const response = await fetch("/api/automated-qa/capture-screenshots", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to create QA job");
      }
      ...
      setQaStatus("Automated QA failed");
      toast.error(`Automated QA failed: ${error.message}`);
    } finally {
      setAutomatedQARunning(false);
    }
  };

  const handleScreenshotsCaptured = async (screenshotUrls: string[]) => {
    // Ensure a QA job exists; create one if missing to avoid blocking
    let jobId = qaJobId;
    if (!jobId) {
      try {
        const resp = await fetch("/api/automated-qa/capture-screenshots", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ glbUrl: asset?.glb_link, assetId: asset?.id }),
        });
        const json = await resp.json();
        if (!resp.ok || !json.qaJobId)
          throw new Error(json.error || "Failed to create QA job");
        jobId = json.qaJobId;
        setQaJobId(jobId);
      } catch (e: any) {
        setQaError(e.message || "No QA job ID available");
        return;
      }
    }

    setQaStatus("Screenshots captured. Updating QA job...");

    try {
      // Get model stats from the statistics card
      const modelStats = {
        triangles: 0, // Will be populated from ModelStatisticsCard
        vertices: 0,
        meshes: 0,
        materials: 0,
      ...
      // Update QA job with screenshots
      const response = await fetch("/api/automated-qa/capture-screenshots", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to update QA job");
      }

      setQaStatus("Screenshots uploaded successfully. Starting QA analysis...");
      toast.success("Screenshots captured and uploaded successfully!");

      // Trigger the QA system with screenshots and reference images
      setQaStatus("Starting QA analysis...");

      try {
        // Use parsed reference images state; fall back to asset fields if needed
        const referencesForQA = (
          referenceImages && referenceImages.length > 0
            ? referenceImages
            : (asset as any)?.reference_images ||
              (asset as any)?.reference ||
              []
        ) as string[];

        if (!referencesForQA || referencesForQA.length === 0) {
          throw new Error("No reference images available for QA comparison");
        }

        // Get model statistics from the ModelStatisticsCard
        const modelStats = {
          meshCount: 0, // Will be populated from ModelStatisticsCard
          materialCount: 0,
          vertices: 0,
          triangles: 0,
          doubleSidedCount: 0,
          ...
          fileSize: 0,
          requirements: {
            maxTriangles: 150000, // From your guidelines
            maxMaterials: 5,
        ...
        const qaResponse = await fetch("/api/qa-jobs", {
          ...
          body: JSON.stringify({
            renders: screenshotUrls,
            references: referencesForQA,
            modelStats,
          }),
        });
        ...
        if (!qaResponse.ok) {
          throw new Error(qaData.error || "Failed to create QA job");
        }

        setQaAnalysisJobId(qaData.jobId);
        setQaStatus(
          `QA job created (ID: ${qaData.jobId}). Analysis in progress...`
        );
        // start polling for results
        if (qaPollTimerRef.current) clearInterval(qaPollTimerRef.current);
        qaPollTimerRef.current = setInterval(async () => {
          try {
            const statusRes = await fetch(`/api/qa-jobs?jobId=${qaData.jobId}`);
            const statusJson = await statusRes.json();
            if (!statusRes.ok) return;
            if (statusJson.status === "complete") {
              clearInterval(qaPollTimerRef.current!);
              qaPollTimerRef.current = null;
              setQaStatus("QA analysis complete");
              setQaResults(statusJson.qaResults || null);
            } else if (statusJson.status === "failed") {
              clearInterval(qaPollTimerRef.current!);
              qaPollTimerRef.current = null;
              setQaError(statusJson.error || "QA failed");
              setQaStatus("QA analysis failed");
            } else {
              setQaStatus(
                `Analysis in progress... (queue: ${
                  statusJson.queueInfo?.position ?? 0
                }, active: ${statusJson.queueInfo?.activeJobs ?? 0})`
              );
            }
          } catch (e) {}
        }, 2000);
        ...
        // TODO: Poll for QA results or implement webhook
        // For now, we'll just show that the job was created
      } catch (error: any) {
        console.error("QA system error:", error);
        setQaError(error.message);
        setQaStatus("QA analysis failed");
        toast.error(`QA Error: ${error.message}`);
      }
    } catch (error: any) {
      console.error("Failed to update QA job:", error);
      setQaError(error.message);
      setQaStatus("Failed to update QA job");
      toast.error(`Failed to update QA job: ${error.message}`);
    }
  };

  const handleQAError = (error: string) => {
    setQaError(error);
    setQaStatus("QA process failed");
    toast.error(`QA Error: ${error}`);
  };
        ...
        <div className="flex flex-1 overflow-hidden bg-background">
          ...
          <div className="w-[620px] max-w-full flex flex-col bg-background shadow-lg border border-border/50 p-6">
            ...
            <div className="mb-6 p-4 bg-muted/30 rounded-lg border border-border/50">
              ...
              {/* Automated QA Status */}
              {(automatedQARunning || qaStatus || qaError) && (
                <div className="mt-4 p-3 rounded-lg border bg-muted/50">
                  ...
                  {automatedQARunning && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <span>{qaStatus || "Processing..."}</span>
                    </div>
                  )}

                  {qaStatus && !automatedQARunning && (
                    <div className="space-y-2">
                      <div className="text-sm text-green-600">{qaStatus}</div>
                      {/* Show latest QA results if present */}
                      {qaResults && (
                        <div className="text-sm">
                          <div className="font-medium">
                            Status: {qaResults.status || "Unknown"}
                          </div>
                          {qaResults.similarityScores && (
                            <div className="text-muted-foreground">
                              Overall:{" "}
                              {qaResults.similarityScores?.overall ?? "-"}% |
                              Silhouette:{" "}
                              {qaResults.similarityScores?.silhouette ?? "-"}% |
                              Proportion:{" "}
                              {qaResults.similarityScores?.proportion ?? "-"}% |
                              Color/Material:{" "}
                              {qaResults.similarityScores?.colorMaterial ?? "-"}
                              %
                            </div>
                          )}
                          {qaResults.summary && (
                            <div className="mt-1">{qaResults.summary}</div>
                          )}
                          {Array.isArray(qaResults.differences) &&
                            qaResults.differences.length > 0 && (
                              <ul className="mt-2 list-disc pl-5">
                                {qaResults.differences
                                  .slice(0, 5)
