...
const RenderOptionsPanel: React.FC<RenderOptionsPanelProps> = ({
  ...
  const [pendingJobs, setPendingJobs] = useState<
    Array<{ jobId: string; modelName: string }>
  >([]);
  ...
  const isBusyForCurrentModel =
    isSubmitting ||
    isRenderingSelected ||
    pendingJobs.some(
      (p) => p.modelName === currentModelName && currentModelName
    );
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      const savedResolution = localStorage.getItem(
        "charpstar:renderSettings:resolution"
      );
      const savedAspectRatio = localStorage.getItem(
        "charpstar:renderSettings:aspectRatio"
      );
      const savedBackgroundMode = localStorage.getItem(
        "charpstar:renderSettings:backgroundMode"
      );
      const savedBackgroundColor = localStorage.getItem(
        "charpstar:renderSettings:backgroundColor"
      );
      const savedFormat = localStorage.getItem(
        "charpstar:renderSettings:format"
      );
      ...
      if (savedBackgroundMode)
        setBackgroundMode(savedBackgroundMode as BackgroundMode);
  ...
  React.useEffect(() => {
    ...
    try {
      localStorage.setItem(
        "charpstar:renderSettings:views",
  ...
  const computeBlocked = async () => {
    try {
      ...
      const currentVariant = isModularMode
        ? null
        : (modelViewerRef.current as any)?.variantName || null;
      ...
      const active = items.some((it: any) => {
        ...
        const sameVariant =
          (it.variantName || null) === (currentVariant || null);
        const st = String(it.status || "unknown");
        return (
          sameModel && sameVariant && st !== "completed" && st !== "failed"
        );
      });
      return active;
    } catch {
  ...
  React.useEffect(() => {
    ...
    try {
      window.addEventListener("charpstar:renderJobStarted", onStarted as any);
    } catch {}
          ...
          setTimeout(() => {
            try {
              window.dispatchEvent(
                new CustomEvent("charpstar:renderPendingRemove", {
                  detail: { modelName: removedModelName, jobId },
                })
              );
            } catch {}
          }, 0);
    ...
    try {
      window.addEventListener("charpstar:renderFinished", onFinished as any);
    } catch {}
    try {
      window.addEventListener("charpstar:renderCompleted", onFinished as any);
    } catch {}
    ...
    return () => {
      ...
      try {
        window.removeEventListener(
          "charpstar:renderFinished",
          onFinished as any
        );
      } catch {}
      try {
        window.removeEventListener(
          "charpstar:renderCompleted",
          onFinished as any
        );
      } catch {}
      clearInterval(t);
  ...
  const handleStartRender = async () => {
    ...
    const views = selectedViews.map((viewName) => {
      const preset =
        cameraPresets.find((p) => p.name === viewName) || cameraPresets[0];
    ...
    try {
      ...
      if (jobId) {
        try {
          window.dispatchEvent(
            new CustomEvent("charpstar:renderJobStarted", {
              detail: { clientName, jobId },
            })
          );
          window.dispatchEvent(
            new CustomEvent("charpstar:renderPendingAdd", {
              detail: { modelName: payload.modelName, jobId },
            })
          );
        } catch {}
        setPendingJobs((prev) => [
          ...prev,
          { jobId, modelName: payload.modelName },
        ]);
      }
    } catch (e) {
  ...
  const handleStartRenderSelected = async () => {
    ...
    try {
      ...
      for (const asset of selectedAssets) {
        ...
        if (res.ok && json?.jobId) {
          try {
            window.dispatchEvent(
              new CustomEvent("charpstar:renderJobStarted", {
                detail: { clientName, jobId: json.jobId },
              })
            );
          } catch {}
          setPendingJobs((prev) => [
            ...prev,
            { jobId: json.jobId, modelName: payload.modelName },
          ]);
        } else {
          console.warn(
            "Failed to queue render for asset:",
            asset?.product_name || filename,
            json?.error
          );
        }
      }
      ...
      alert(
        "Failed to queue selected renders: " +
          (e instanceof Error ? e.message : String(e))
      );
    } finally {
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row bg-gray-50">
      ...
      <div className="w-full lg:w-1/2 m-2 rounded-lg bg-white border-1 border-gray-200 flex flex-col">
        {/* Settings Header */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4 lg:gap-5">
              ...
              <div className="sm:col-span-1">
                <label className="text-[10px] sm:text-xs font-semibold text-gray-700 uppercase tracking-wide mb-2 sm:mb-3 block">
                  Camera Angles (
                  <span suppressHydrationWarning>{selectedViews.length}</span>)
                </label>
              ...
              <div className="sm:col-span-1">
                ...
                <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                  {[
                    ...
                    const isSelected =
                      resolution === value && aspectRatio === ratio;
                    return (
                      <button
                        key={`${value}-${ratio}`}
                        onClick={() => {
                      ...
                      >
                        ...
                        <span
                          className={`text-[8px] sm:text-[9px] ${isSelected ? "text-gray-300" : "text-gray-500"}`}
                        >
                          {sublabel}
                        </span>
                      </button>
              ...
              <div className="sm:col-span-1">
                ...
                <div className="space-y-1.5 sm:space-y-2">
                  <button
                    onClick={() =>
                      canUseTransparent && setBackgroundMode("transparent")
                    }
              ...
              <div className="col-span-2 sm:col-span-1">
                ...
                <div>
                  <div
                    className={`mb-2 sm:mb-3 ${isColorPickerDisabled ? "opacity-40 pointer-events-none" : ""}`}
                  >
                  ...
                  >
                    <div className="text-[9px] sm:text-[10px] font-semibold text-gray-600 uppercase tracking-wide mb-1.5 sm:mb-2">
                      Quick
                    </div>
        ...
        {/* Bottom Action Bar */}
        <div className="p-2 sm:p-2 lg:p-4 border-t border-gray-200">
                ...
                {isBlocked ? (
                  <TooltipContent>
                    <div className="text-sm">
                      Render already in progress for this model
                    </div>
                  </TooltipContent>
                ) : (
                  <TooltipContent>
                    <div className="text-sm">
                      Render the current variant with selected settings
                    </div>
                  </TooltipContent>
                )}
              </Tooltip>
                    ...
                    {isRenderingSelected ? (
                      ...
                      <>
                        ...
                        <span className="hidden md:inline">
                          Render Selected ({selectedAssets.length})
                        </span>
            ...
            <div className="flex items-center justify-center h-full p-4 sm:p-6 text-center">
              <div>
                <div className="text-xs sm:text-sm text-gray-500">
                  Select a model to view render history
                </div>
              </div>
            </div>
          )}
