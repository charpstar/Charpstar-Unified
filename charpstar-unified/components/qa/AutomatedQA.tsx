  ...
  const handleScreenshotsCaptured = async (
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to submit QA job: ${response.status} ${response.statusText}`
        );
      }
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Our AI-powered QA system will automatically capture screenshots of
              your 3D model and compare them against reference images to ensure
      ...
      {qaState === "analyzing" && qaJobId && (
        <QAResults jobId={qaJobId} onRetry={handleRetry} onClose={onCancel} />
      )}
      ...
      {qaState === "complete" && qaJobId && (
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            {onCancel && <Button onClick={onCancel}>Continue</Button>}
          </div>
        </div>
      )}
    </div>
