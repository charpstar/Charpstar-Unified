...
// Note: ApprovalHistory uses Supabase directly, so it's not available for external reviewers
...
export default function ReviewPage() {
  ...
  // Notes state (not available for external reviewers, but keep for UI compatibility)
  ...
  // Reference image URL state
  const [referenceImageUrl, setReferenceImageUrl] = useState("");
  const [showUrlDialog, setShowUrlDialog] = useState(false);

  // Undo deletion state (not available for external reviewers)
  const [lastDeletedAnnotation] = useState<string | null>(null);
  const [isUndoing] = useState(false);
    ...
    async function fetchAsset() {
      ...
      try {
        const response = await fetch(
          `/api/shared-reviews/${token}/assets/${assetId}`
        );

        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(
            errorData.error || `Failed to load asset (${response.status})`
          );
        }
        ...
        setClientViewerType(data.clientViewerType);

        // Set revision count from asset
        setRevisionCount(data.asset.revision_count || 0);

        // Parse reference images
        if (data.asset.reference) {
          setReferenceImages(
            Array.isArray(data.asset.reference)
  ...
  useEffect(() => {
    async function fetchAnnotations() {
      ...
      try {
        const response = await fetch(
          `/api/shared-reviews/${token}/annotations?asset_id=${assetId}`
        );
        const data = await response.json();

        if (response.ok) {
          // Identify external reviewer annotations by checking created_by matches invitation.created_by
          // and asset_id is in invitation.assetIds (since asset_annotations doesn't have metadata column)
          const annotationsWithMetadata = (data.annotations || []).map(
            (annotation: Annotation) => {
              // Check if this annotation is from an external reviewer via this invitation
              const isExternalReviewAnnotation =
                invitation &&
                annotation.created_by === invitation.created_by &&
                invitation.assetIds?.includes(annotation.asset_id);

              if (isExternalReviewAnnotation) {
                return {
                  ...annotation,
                  profiles: {
          ...
          setAnnotations(annotationsWithMetadata);
        } else {
  ...
  useEffect(() => {
    async function fetchComments() {
      ...
      try {
        const response = await fetch(
          `/api/shared-reviews/${token}/comments?asset_id=${assetId}`
        );
        const data = await response.json();

        if (response.ok) {
          // Identify external reviewer comments by checking created_by matches invitation.created_by
          // and asset_id is in invitation.asset_ids (since asset_comments doesn't have metadata column)
          const commentsWithMetadata = (data.comments || []).map(
            (comment: any) => {
              // Check if this comment is from an external reviewer via this invitation
              const isExternalReviewComment =
                invitation &&
                comment.created_by === invitation.created_by &&
                invitation.assetIds?.includes(comment.asset_id);

              if (isExternalReviewComment) {
                return {
                  ...comment,
                  profiles: {
                    email: invitation?.recipientEmail,
  ...
  const saveAnnotation = async () => {
    if (!selectedAnnotation || !token) return;
    ...
    try {
      const response = await fetch(`/api/shared-reviews/${token}/annotations`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        // Add metadata for display (external reviewer info)
  ...
  const updateAnnotation = async (annotationId: string) => {
    if (!editComment.trim() || !token) return;
  ...
  const deleteAnnotation = async (annotationId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/shared-reviews/${token}/annotations?id=${annotationId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const deleteMultipleAnnotations = async (annotationIds: string[]) => {
    ...
    try {
      const promises = annotationIds.map((id) =>
        fetch(`/api/shared-reviews/${token}/annotations?id=${id}`, {
          method: "DELETE",
        })
      );
  ...
  const submitInlineEdit = async (annotationId: string) => {
    if (!inlineEditComment.trim() || !token) return;

    try {
      const annotation = annotations.find((ann) => ann.id === annotationId);
      if (!annotation || !canEditOrDeleteAnnotation(annotation)) {
  ...
  const handleUploadReferenceImages = async (files: FileList) => {
    try {
      ...
      // External reviewers cannot update reference images
      toast.error(
        "Reference image upload is not available for external reviewers"
      );
      return;
    } catch (error) {
  ...
  const handleAddReferenceImageUrl = async () => {
    ...
    try {
      ...
      // External reviewers cannot update reference images
      toast.error(
        "Reference image URL addition is not available for external reviewers"
      );
      return;
    } catch (error) {
  ...
  // External reviewers cannot update status or revisions directly
  // This function is a stub for external reviewers
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const _updateAssetStatusWithRevision = async (
    _newStatus: string,
    _revisionNumber: number
  ) => {
    if (!assetId || !token) return;

    // External reviewers use API endpoint for status updates
    try {
      // Use the shared review submit API for status updates
          ...
          responses: [
            {
              assetId,
              action:
                _newStatus === "approved_by_client" ? "approve" : "revision",
            },
          ],
        }),
      });
      ...
      // Update local state
      setAsset((prev) => (prev ? { ...prev, status: _newStatus } : null));
      if (_revisionNumber) {
        setRevisionCount(_revisionNumber);
      }
  ...
  const hasClientFeedback = () => {
    ...
    // Check if there are any annotations created by the current reviewer since the last revision
    ...
    // Check if there are any comments created by the current reviewer since the last revision
  ...
  // Note: External reviewers are treated as clients, so this won't be used
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const _hasQAFeedback = () => {
    // External reviewers don't have QA role, so this always returns false
    return false;
  };

  // Comprehensive feedback validation for external reviewers (treated as clients)
  const hasValidFeedback = () => {
    // External reviewers are always treated as clients
  ...
  const updateAssetStatus = async (newStatus: string, skipDialog = false) => {
    ...
    if (
      ...
      // External reviewers are treated as clients, show revision dialogs
    ...
    try {
      // External reviewers use API endpoint for status updates
      // The submit API handles all the status updates
      const response = await fetch(`/api/shared-reviews/${token}/submit`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...
          responses: [
            {
              assetId,
              action:
                newStatus === "approved_by_client" ? "approve" : "revision",
            },
          ],
        }),
      });
      ...
      if (newStatus === "revisions" || newStatus === "client_revision") {
        ...
        // Refresh annotations when status changes to revisions
        try {
  ...
  const handleRevisionConfirm = async () => {
    ...
    try {
      ...
      // Use API endpoint to submit revision
      const response = await fetch(`/api/shared-reviews/${token}/submit`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  // External reviewers can always add annotations/comments
  const isFunctionalityDisabled = () => false;

  // Permission helper functions for token-based system
  const canEditOrDeleteAnnotation = (annotation: Annotation) => {
    // External reviewers can only edit/delete their own annotations
    // Check by created_by matching invitation.created_by and asset_id in invitation
  ...
  const canEditOrDeleteComment = (comment: any) => {
    // External reviewers can only edit/delete their own comments
    // Check by created_by matching invitation.created_by and asset_id in invitation
    if (!invitation) return false;
    return (
      comment.created_by === invitation.created_by &&
  ...
  // For external reviewers, treat them as "client" role
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const _getCurrentUserRole = () => "client";
  ...
  // Function to restore a revision - not available for external reviewers
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const restoreRevision = async (_revisionNumber: number) => {
    toast.error("Revision restoration is not available for external reviewers");
  };

  // Fetch revision history - stub for external reviewers
  const fetchRevisionHistory = async () => {
    // External reviewers don't have access to revision history
    setRevisionHistory([]);
  };

  // Get revision items from history - stub for external reviewers
  ...
  // Get revision number for an item based on creation date
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const getRevisionForItem = (_createdAt: string): number => {
    // For external reviewers, we can't determine revision from date
    // Return 0 as default
    return 0;
  };

  // Undo last deletion - not available for external reviewers
  const undoLastDeletion = async () => {
    toast.error("Undo feature is not available for external reviewers");
  };

  // Update modeler end time - not available for external reviewers
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const _updateModelerEndTime = async (_assetId: string) => {
    // External reviewers can't update modeler times
    return;
  };
  ...
  // Permissions are handled by the API based on token
  ...
  const addComment = async () => {
    if (!newCommentText.trim() || !assetId || !token) return;

    try {
      const response = await fetch(`/api/shared-reviews/${token}/comments`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        ...
        setComments((prev) => [commentWithMetadata, ...prev]);
        setNewCommentText("");
  ...
  const submitCommentInlineEdit = async (commentId: string) => {
    if (!inlineEditCommentText.trim() || !token) return;
    ...
    try {
      const response = await fetch(`/api/shared-reviews/${token}/comments`, {
        ...
        body: JSON.stringify({
          id: commentId,
          comment: inlineEditCommentText.trim(),
        }),
      });
  ...
  const deleteComment = async (commentId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/shared-reviews/${token}/comments?id=${commentId}`,
        {
          method: "DELETE",
        }
      );
  ...
  // Notes functions - not available for external reviewers
  const addNote = async () => {
    toast.error("Notes feature is not available for external reviewers");
  };

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const updateNote = async (_noteId: string) => {
    toast.error("Notes feature is not available for external reviewers");
  };
  ...
  const submitReply = async () => {
    if (!replyingTo || !replyText.trim() || !assetId || !token) return;
    ...
    try {
      if (replyingTo.type === "annotation") {
        ...
        if (!parent) return;
        const response = await fetch(
          `/api/shared-reviews/${token}/annotations`,
          {
            method: "POST",
        ...
        setComments((prev) => [replyWithMetadata, ...prev]);
        toast.success("Reply posted");
      }
      setReplyText("");
      setReplyingTo(null);
  ...
  // Toggle comment priority - not available for external reviewers
  ...
  return (
    <>
      <div className="h-[calc(100vh-140px)] w-full flex flex-col bg-muted overflow-hidden">
        {/* Enhanced Header */}
        <div className="bg-background/95 backdrop-blur-xl border-b border-border/50 shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 gap-3 sm:gap-0">
              ...
              <div className="flex items-center gap-3 sm:gap-6 min-w-0 flex-1">
                <div className="min-w-0 flex-1">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                    ...
                    {/* Notes feature not available for external reviewers */}
                    <Button
                      variant="outline"
                      size="xxs"
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                      <Badge
                        ...
                        className={`text-xs font-medium ${
                          getStatusDisplay(asset?.status, "client").className
                        }`}
                      >
                        {getStatusDisplay(asset?.status, "client").label}
                      </Badge>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {/* External reviewers can approve */}
                <Button
                ...
                {/* External reviewers can request revisions */}
                <Button
                  onClick={() => {
                    updateAssetStatus("client_revision");
                  }}
                  disabled={
                    asset?.status === "revisions" ||
                    asset?.status === "client_revision" ||
        ...
        <div className="flex flex-col xl:flex-row flex-1 overflow-hidden min-h-0">
          ...
          <div className="w-full xl:w-[510px] max-w-full flex flex-col bg-background border-l border-border/20 p-3 sm:p-6 h-full min-h-0 overflow-y-auto">
            {/* Tab Navigation */}
                            ...
                            <Button
                              ...
                              onClick={async (e) => {
                                ...
                                try {
                                  ...
                                  // External reviewers cannot delete reference images
                                  toast.error(
                                    "Reference image deletion is not available for external reviewers"
                                  );
                                  return;
                                } catch (error) {
                                  console.error(
                        ...
                        item.type === "annotation" ? (
                          ...
                          >
                            <div className="flex items-start justify-between mb-4">
                              ...
                              {/* Priority feature not available for external reviewers */}
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <Button
                                    variant="ghost"
        ...
        <DialogContent className="w-[95vw] sm:w-full max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-6 text-center">
              <p className="text-sm text-muted-foreground">
                Status history is not available for external reviewers.
              </p>
            </div>
          </div>
        </DialogContent>
